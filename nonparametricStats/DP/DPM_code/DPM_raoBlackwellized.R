...
DPM_raoBlackwellized = function(iter_num, M, N, mu0, sigma0, sigma) {
  ...
  # count_category = c()
  # for (num in 1:length(theta_star)) {
	 #  count_category = c(count_category, sum(thetas == theta_star[num]))  	
  # }
  ...
  for (k in length(category_unique)) {
  	for (loop in 1:N) {
  ...
  for (j in 1:iter_num) {
  	for (i in 1:N) {
  		...
  		for (temp_cat in temp_category) {
  			...
  			p = c(p, n_c / (N-1+M) * dnorm(temp_y$y, mu_theta, sigma_theta + sigma))
  		}
  		p = c(p, M / (N-1+M) * dnorm(temp_y$y, mu0, sigma0 + sigma))
  		p = p / sum(p)
  		cat('p:', p, '\n')
  		cat('length: ', length(p), '\n')
  		cat('K+1: ', K+1, '\n')
  		...
  		category_unique = unique(DT$category)
  		for (k in length(category_unique)) {
		  	for (loop in 1:N) {
		  		if (DT[loop,]$category == category_unique[k]) {
		  			DT[loop,]$category = k
